  ...
  ({
    ...
    return (
      <ReasoningContext.Provider value={{ isStreaming, isOpen, setIsOpen, duration }}>
        <Collapsible
          className={cn(
            'not-prose bg-muted/30 mb-4 rounded-lg border border-dashed p-3 backdrop-blur-sm',
            'hover:bg-muted/40 transition-all duration-200',
            isStreaming && 'animate-pulse border-blue-200 dark:border-blue-800',
            className,
          )}
...
export const ReasoningTrigger = memo(({ className, children, ...props }: ReasoningTriggerProps) => {
  ...
  return (
    <CollapsibleTrigger
      className={cn(
        'text-muted-foreground/80 flex items-center gap-2 text-sm font-medium',
...
export const ReasoningContent = memo(({ className, children, ...props }: ReasoningContentProps) => (
  <CollapsibleContent
    className={cn(
      'text-muted-foreground/70 mt-3 text-sm',
    ...
    {...props}
  >
    <div className="bg-muted/20 rounded-md border-l-2 border-blue-200 p-3 dark:border-blue-800">
      <Response className="grid gap-2 font-mono text-xs leading-relaxed opacity-80">
        {children}
      </Response>
    </div>
  </CollapsibleContent>
